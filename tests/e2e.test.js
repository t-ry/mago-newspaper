...
test(
  ...
  async () => {
    ...
    try {
      ...
      await page
        .getByRole("heading", { name: "今週号が、できました。" })
        .waitFor();
      await page.frameLocator("iframe").locator(".family-note").waitFor();
      assert.equal(
        await page
          .getByRole("button", { name: "この紙面を承認する" })
          .isDisabled(),
        true,
      );
      ...
      await page
        .getByRole("button", { name: "A4 PDFをダウンロード" })
        .waitFor();
      ...
      const doc = await PDFDocument.load(
        await readFile("test-results/newspaper.pdf"),
      );
      assert.equal(doc.getPageCount(), 1);
      const size = doc.getPage(0).getSize();
      assert.ok(Math.abs(size.width - 595.28) < 2);
      assert.ok(Math.abs(size.height - 841.89) < 2);
      await page.screenshot({
        path: "test-results/newspaper-desktop.png",
        fullPage: true,
      });
      ...
      await page
        .getByLabel("住所", { exact: true })
        .fill("東京都千代田区 デモ用住所");
      await page.getByRole("button", { name: "模擬注文を確定する" }).click();
      await page
        .getByRole("heading", { name: "家族へのお届けを、体験しました。" })
        .waitFor();
      await page.setViewportSize({ width: 390, height: 844 });
      await page.screenshot({
        path: "test-results/newspaper-mobile.png",
        fullPage: true,
      });
      assert.ok(
        await page.evaluate(
      ...
      await page
        .getByRole("button", { name: "新しい号をつくる", exact: true })
        .last()
        .click();
      await page.screenshot({
