...
- スマホの「カメラで撮る」から背面カメラを開き、撮影結果をそのまま追加。既存のJPEG / PNG / WebPも最大12枚まとめて選択可能（1枚10MB以内）。
- 撮影・選択した画像はブラウザで最大1600pxへ縮小し、位置情報を含まないJPEGへ再エンコード。
...
![スマホでカメラから直接取り込む画面](docs/assets/camera-mobile.png)
