...
実郵送アダプターは事業者決定後に追加し、MVPでは模擬注文を使う。OrcaRouterキーはローカル設定済みで、認証・モデル一覧・無料テキスト生成まで実接続を確認した。画像付き生成は画像対応モデルのクレジット反映後に再検証する。GitHubは公開済み。Qiitaは原稿と投稿スクリプトを用意し、`write_qiita`権限のトークン設定後に公開する。
