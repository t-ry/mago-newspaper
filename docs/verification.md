...
- Cloud Run用Dockerイメージ: ビルド成功。非rootユーザー、Node.js 20、Playwright Chromiumを含む構成を確認。
- Firebase Hosting向け公開元検証: 設定した`PUBLIC_ORIGIN`からのAPI呼び出しを許可し、未知の外部Originを403で拒否。
...
- Firebase / Cloud Runへの実デプロイとスマホ実機確認は、専用プロジェクト作成・課金設定・CLI再ログイン後に実施。
